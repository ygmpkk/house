...
tc_binds_and_then top_lvl combiner (MonoBind bind sigs is_rec) do_next
  =   	-- BRING ANY SCOPED TYPE VARIABLES INTO SCOPE
	...
	NotTopLevel	-- For nested bindings we must do the bindInstsOfLocalFuns thing
	     ...
	     ->		-- NB: we want to keep non-recursive things non-recursive
		...
		returnM (
			...
			-- NB: the binds returned by bindInstsOfLocalFuns aren't 
			-- guaranteed in dependency order (though we could change that); 
		...
		getLIE (
		    tcBindWithSigs top_lvl bind sigs is_rec	`thenM` \ (poly_binds, poly_ids) ->
		    tc_body poly_ids 				`thenM` \ (prag_binds, thing) ->
		    returnM (poly_ids, poly_binds `andMonoBinds` prag_binds, thing)
                )   `thenM` \ ((poly_ids, extra_binds, thing), lie) ->

		bindInstsOfLocalFuns lie poly_ids	`thenM` \ lie_binds ->
		returnM (
			combiner (mkMonoBind Recursive (extra_binds `andMonoBinds` lie_binds))
				 thing )
  where
