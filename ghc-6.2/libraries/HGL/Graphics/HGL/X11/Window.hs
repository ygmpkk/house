...
import Graphics.HGL.X11.Utilities( modMVar, modMVar_ )
import Graphics.HGL.X11.Event
import qualified Graphics.HGL.X11.Events as E
...
openWindowEx name pos size redrawMode tickRate = do
  ...
  win <- newWindow display window fg_color text_gc pen_gc brush_gc ((x,y),(w,h)) redraw tickRate
...
newWindow :: X.Display -> X.Window -> X.Pixel -> X.GC -> X.GC -> X.GC -> ((X.Position,X.Position),(X.Dimension,X.Dimension)) -> RedrawStuff -> Maybe T.Time -> IO Window
