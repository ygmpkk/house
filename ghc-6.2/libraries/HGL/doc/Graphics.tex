...
\module{GraphicsUtils} module provides these built in colors as
...
(These are both exported from the \module{GraphicsUtils} module.)
...
exported from \module{GraphicsCore} and \module{GraphicsUtils}.
\module{GraphicsUtils} reexports all symbols exported by
\module{GraphicsCore} and it is expected that most users will only
import \module{GraphicsUtils}; the \module{GraphicsCore} interface is
aimed solely at those wishing to use the graphics library as a base on
which to build their own library or who find the
\module{GraphicsUtils} interface inappropriate for their needs.
