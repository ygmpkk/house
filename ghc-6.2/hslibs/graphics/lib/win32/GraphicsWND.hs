...
import GraphicsDraw( DrawFun, setDefaults, withDC )
import GraphicsKey
import GraphicsTypes
import GraphicsUtilities(safeTry, Exception)
import IOExts
import IO(try)
...
createWND name wndProc posn size style menu = do
  ...
  hwnd <- createWindowEx 
	    ...
	    name
	    style
	    (fmap (toInt.fst) posn)   (fmap (toInt.snd) posn)   -- x y
	    (fmap (toInt.fst) mbSize) (fmap (toInt.snd) mbSize) -- w h
	    Nothing                           -- parent
	    menu
	    mainInstance
	    wndProc
...
-}
 ...
 where
  ...
  char wParam = do
     	send (Char { char = toEnum (toInt wParam) })
	return 0
