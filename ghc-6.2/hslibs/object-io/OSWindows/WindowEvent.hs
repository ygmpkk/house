...
import StdWindowAttribute(isWindowKeyboard, getWindowKeyboardAtt, isWindowMouse, getWindowMouseAtt)
import WindowAccess
import WindowCrossCall_12
import OSWindow(fromOSscrollbarRange, osScrollbarsAreVisible)
import Cutil_12 (int2addr)
...
--					appIOEnv (ioStSetDevice (WindowSystemState windows));
		...
		windowEvent2 ioState schedulerEvent@(ScheduleMsgEvent recLoc)
			| ioStGetIOId ioState /= rlIOId recLoc || rlDevice recLoc /= WindowDevice =
				return (False,Nothing,schedulerEvent)
			| otherwise =
				let
					(_,wDevice)	= ioStGetDevice WindowDevice ioState
					windows		= windowSystemStateGetWindowHandles wDevice
					found 		= hasWindowHandlesWindow (toWID (rlParentId recLoc)) windows
...
filterCOMBOSELECT :: OSWindowMetrics -> OSEvent -> WindowHandles ps -> IO (Bool,Maybe [Int],Maybe DeviceEvent)
filterCOMBOSELECT _ (CrossCallInfo {ccMsg=ccWm,p1=wPtr,p2=cPtr,p3=index}) windows
	| not found
		= return (False,Nothing,Nothing)
	| not (getWindowStateHandleSelect wsH)
		= return (True,Nothing,Nothing)
	| otherwise =
		let
			wids			= getWindowStateHandleWIDS wsH
		  	itemNr			= getPopUpControlItemNr cPtr wsH
	...
	where
		...
		getPopUpControlItemNr :: OSWindowPtr -> WindowStateHandle ps -> Int
		getPopUpControlItemNr cPtr wsH@(WindowStateHandle _ (Just (WindowLSHandle {wlsHandle=wH@(WindowHandle {whItems=whItems})}))) =
			case getPopUpControlsItemNr cPtr whItems of
				(_,itemNr) -> itemNr
			where
				getPopUpControlsItemNr :: OSWindowPtr -> [WElementHandle ls ps] -> (Bool,Int)
				getPopUpControlsItemNr cPtr (itemH:itemHs) =
					case getPopUpControlItemNr cPtr itemH of
						(True,itemNr) 	-> (True,itemNr)
						(False,itemNr) 	-> getPopUpControlsItemNr cPtr itemHs
					where
						getPopUpControlItemNr :: OSWindowPtr -> WElementHandle ls ps -> (Bool,Int)
						getPopUpControlItemNr cPtr itemH@(WItemHandle {wItemPtr=wItemPtr,wItemNr=wItemNr,wItemKind=wItemKind,wItemSelect=wItemSelect,wItemShow=wItemShow,wItems=wItems})
							| cPtr==wItemPtr
								= (True,if wItemKind==IsPopUpControl && wItemSelect && wItemShow then wItemNr else 0)
							| wItemShow
								= getPopUpControlsItemNr cPtr wItems
							| otherwise
								= (False,0)
						...
						getPopUpControlItemNr cPtr (WExtendLSHandle ls1 itemHs)
							= getPopUpControlsItemNr cPtr itemHs

						getPopUpControlItemNr cPtr (WChangeLSHandle ls1 itemHs)
							= getPopUpControlsItemNr cPtr itemHs
...
filterKEYBOARD context _ (CrossCallInfo {p1=wptr,p2=cptr,p3=keycode,p4=state,p5=mods}) windows
	= let (found,wsH,_)              = getWindowHandlesWindow (toWID wptr) windows
	  ...
	  then return (False,Nothing,Nothing)
		  ...
		  let (ok,key,newInputTrack) = okWindowKeyboardState keycode state mods wsH inputTrack
		  let iocontext1 = ioContextSetInputTrack newInputTrack iocontext
		  writeIORef context iocontext1
		  let deviceEvent = if ok then (Just (WindowKeyboardAction (WindowKeyboardActionInfo {wkWIDS=wids,wkKeyboardState=key}))) else Nothing
	...
	where
		condition :: OSWindowPtr -> Cond (WElementHandle ls ps)
		condition cptr itemH = cptr==wItemPtr itemH

		okWindowKeyboardState :: Int -> Int -> Int -> (WindowStateHandle ps) -> Maybe InputTrack
								 -> (Bool,KeyboardState,Maybe InputTrack)
		okWindowKeyboardState keyCode state mods wsH@(WindowStateHandle _ (Just (WindowLSHandle {wlsHandle=(WindowHandle {whKind=whKind,whWindowInfo=whWindowInfo,whAtts=whAtts})}))) inputTrack
		    | whKind==IsDialog = (False,undefined,inputTrack)
		    | trackingKeyboard wptr 0 inputTrack =			-- Window is already handle Key(Repeat/Up)
			    if isDownKey then (False,undefined,inputTrack)	-- Ignore all key down events
			    else if pressState==KeyUp then (okKeyboardAtt,keystate,untrackKeyboard inputTrack)	-- Clear keyboard tracking
			    else (okKeyboardAtt,keystate,inputTrack)
		    | isDownKey = (okKeyboardAtt,keystate,trackKeyboard wptr 0 inputTrack)	-- Key down sets input track
		    | otherwise = (False,undefined,inputTrack)
		    where
		...
		okControlItemNrKeyboardState :: Int -> Int -> Int -> St (WElementHandle ls ps) (Bool,Int,KeyboardState)
		okControlItemNrKeyboardState keycode state mods itemH@(WItemHandle {wItemNr=wItemNr,wItemAtts=wItemAtts})
			...
			| otherwise
				= ((False,0,undefined),itemH)
			where
				noKeyboardAtt = ControlKeyboard (const False) Unable undefined
