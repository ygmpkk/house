module OSWindow ( osControlTitleSpecialChars
		...
		, osGetSliderControlSize
                ...
                , osGetPopUpControlMinWidth, osGetRadioControlItemMinWidth
                , osGetCheckControlItemMinWidth, osGetSliderControlMinWidth
                , osCreateDialog, osCreateWindow, osCreateModalDialog
                , OKorCANCEL(..)
                , osCreateRadioControl, osCreateCheckControl, osCreateEmptyPopUpControl, osCreatePopUpControlItem
                ...
                , osDestroyWindow
                , osDestroyRadioControl, osDestroyCheckControl, osDestroyPopUpControl
		...
		, osSetSliderControlPos, osSetSliderControlSize
                , osSetRadioControl, osSetRadioControlSelect, osSetRadioControlShow
                , osSetRadioControlPos, osSetRadioControlSize
                , osSetCheckControl, osSetCheckControlSelect, osSetCheckControlShow
		...
		, osSetPopUpControlSelect, osSetPopUpControl
                ...
                , OSSystem.OSWindowMetrics(..), ScrollbarInfo(..)
                ) where
...
import Cutil_12(addr2int, int2addr, newCString, free, nullPtr)
import CommonDef
...
osCreateDialog isModal isClosable title pos size behindptr get_focus create_controls update_controls osdinfo control_info
	= do {
		...
		let createcci = rq4Cci ccRqCREATEDIALOG (addr2int textptr) parentptr (if behindptr==osNoWindowPtr then 0 else behindptr) (fromBool isModal)
		in
		do {
...
osCreatePopUpControlItem :: OSWindowPtr -> Int -> Bool -> String -> Bool -> IO Int
osCreatePopUpControlItem parentPopUp pos able title selected
	= do {
		textPtr <- newCString title;
	  	let nisSelected = if selected then 1 else 0
	  	    nisAble = if able then 1 else 0
	  	    addcci = rq5Cci ccRqADDTOPOPUP parentPopUp (addr2int textPtr) nisAble nisSelected pos
	  	in
	  	do {
...
destroycontrol wPtr
	...
	where
		...
		osDestroyControlCallback info@(CrossCallInfo {ccMsg=ccMsg})
			...
			where
				expected = ccMsg `elem`   [ccWmACTIVATE,ccWmBUTTONCLICKED,ccWmCOMBOSELECT,ccWmCOMMAND,ccWmDEACTIVATE
...
osSetRadioControl :: OSWindowPtr -> OSWindowPtr -> OSWindowPtr -> Rect -> IO ()
osSetRadioControl _ current new _
	= winCheckControl new True >> winCheckControl current False
...
osSetCheckControl :: OSWindowPtr -> OSWindowPtr -> Rect -> Bool -> IO ()
osSetCheckControl _ cPtr _ check
	= winCheckControl cPtr check
...
osSetPopUpControl :: OSWindowPtr -> OSWindowPtr -> Rect -> Rect -> Int -> Int -> String -> Bool -> IO ()
osSetPopUpControl _ pPtr _ _ _ new _ _
