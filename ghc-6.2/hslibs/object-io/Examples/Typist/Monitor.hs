...
openMonitor pos monitorId = do
	font <- liftIO (doScreenDraw openDialogFont)
	metrics <- liftIO (doScreenDraw (getFontMetrics font))
	ids <- openIds 2
	openProcesses (monitorProcess pos font metrics monitorId ids)
...
monitorProcess pos font metrics monitorId [wId,tId] =
	...
	--	monitorlook defines the look of the monitor window. 
		...
		monitorlook (Monitor{counts=counts,tracking=tracking}) _ _ = do
			...
			when (not tracking) (drawTotalAndAverage font metrics counts)
