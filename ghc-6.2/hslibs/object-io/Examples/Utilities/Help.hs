...
infoFontName1	= fName sansSerifFontDef
infoFontName2	= fName serifFontDef
normalSize1	= 9
normalSize2	= 12
largeSize1	= 12
largeSize2	= 14
...
showAbout appName helpFile ps = do
	okId <- openId
	fonts <- liftIO (doScreenDraw infoFonts)
	let Fonts{normal=normal} = fonts
	(size,infoText)	<- readInfo about fonts aboutBegin aboutEnd helpFile
	let about = Dialog ("About "++appName)
		(   CustomControl size (look normal infoText) 
...
showHelp helpfile ps = do
	fonts <- liftIO (doScreenDraw infoFonts)
	(size,infoText) <- readInfo help fonts helpBegin helpEnd helpfile
	let window = Window "Help" NilLS
		[ WindowViewSize	size
		, WindowLook		True (look (normal fonts) infoText)
...
infoFonts :: Draw Fonts
infoFonts = do
	normal 		<- selectFont [(infoFontName1,normalSize1),(infoFontName2,normalSize2)] []
	large  		<- selectFont [(infoFontName1,largeSize1 ),(infoFontName2,largeSize2 )] []
	bold   		<- selectFont [(infoFontName1,normalSize1),(infoFontName2,normalSize2)] [boldStyle]
	large_bold 	<- selectFont [(infoFontName1,largeSize1 ),(infoFontName2,largeSize2 )] [boldStyle]
	return (Fonts {normal=normal,large=large,bold=bold,large_bold=large_bold})
	where
		selectFont :: [(String,Int)] -> [FontStyle] -> Draw Font
		selectFont ((fontName,size):prefFonts) style = do
			mb_font <- openFont (FontDef {fName=fontName,fStyles=style,fSize=size})
			(case mb_font of
				Just font -> return font
				Nothing   -> selectFont prefFonts style)
		selectfont [] style = openDefaultFont
