module ControlAccess ( 	getcontrolslayouts, getcontrolsviewsizes
		     ...
		     , 	getpopupitems, getselectedpopupitems
		     ,	getradioitems, getradiocontrolsmarks
		     ,	getcheckitems, getcheckcontrolsmarks
...
import OSWindow(OSWindowMetrics, osGetEditControlText, osScrollbarsAreVisible, osGetCompoundContentRect)
	...
	where
		...
		getlayouts itemH@(WItemHandle {wItemAtts=atts,wItemPos=wItemPos}) layouts =
			...
			where
				...
				(hasAtt,posAtt)	= cselect isControlPos (ControlPos (StdIOCommon.Left,zero)) atts
...
getpopupitems :: [Id] -> OSWindowMetrics -> WIDS -> WindowHandle ls ps -> [(Id,Bool,Maybe [String])] -> IO (WindowHandle ls ps, [(Id,Bool,Maybe [String])])
getpopupitems ids wMetrics wids wH@(WindowHandle {whItems=itemHs}) titles = do
	(ids, itemHs, resizes) <- statemapWElementHandles getpopuptitle ids itemHs titles
	return (wH{whItems=itemHs}, resizes)
	where
		getpopuptitle :: WElementHandle ls ps -> [(Id,Bool,Maybe [String])] -> IO (WElementHandle ls ps, [(Id,Bool,Maybe [String])])
		getpopuptitle itemH@(WItemHandle {wItemKind=itemKind,wItemInfo=info}) titles
			| itemKind /= IsPopUpControl	= return (itemH, titles )
			| otherwise			= return (itemH, titles1)
			where
				itemId		= fromJust (wItemId itemH)
				title		= (itemId,True,Just (map fst (popUpInfoItems (getWItemPopUpInfo info))))
				(_,titles1)	= creplace (eqfst3id itemId) title titles


getselectedpopupitems :: [Id] -> OSWindowMetrics -> WIDS -> WindowHandle ls ps -> [(Id,Bool,Maybe Index)] -> IO (WindowHandle ls ps, [(Id,Bool,Maybe Index)])
getselectedpopupitems ids wMetrics wids wH@(WindowHandle {whItems=itemHs}) indices = do
	(ids, itemHs, indices) <- statemapWElementHandles getselectedpopup ids itemHs indices
	return (wH{whItems=itemHs}, indices)
	where
		getselectedpopup :: WElementHandle ls ps -> [(Id,Bool,Maybe Index)] -> IO (WElementHandle ls ps, [(Id,Bool,Maybe Index)])
		getselectedpopup itemH@(WItemHandle {wItemKind=itemKind,wItemInfo=info}) indices
			| itemKind /= IsPopUpControl	= return (itemH,indices )
			| otherwise			= return (itemH,indices1)
			where
				itemId		= fromJust (wItemId itemH)
				index		= (itemId,True,Just (popUpInfoIndex (getWItemPopUpInfo info)))
				(_,indices1)	= creplace (eqfst3id itemId) index indices


getradioitems :: [Id] -> OSWindowMetrics -> WIDS -> WindowHandle ls ps -> [(Id,Bool,Maybe [String])] -> IO (WindowHandle ls ps, [(Id,Bool,Maybe [String])])
getradioitems ids wMetrics wids wH@(WindowHandle {whItems=itemHs}) titles = do
	(ids, itemHs, titles) <- statemapWElementHandles getradiotitle ids itemHs titles
	return (wH{whItems=itemHs}, titles)
	where
		getradiotitle :: WElementHandle ls ps -> [(Id,Bool,Maybe [String])] -> IO (WElementHandle ls ps, [(Id,Bool,Maybe [String])])
		getradiotitle itemH@(WItemHandle {wItemKind=itemKind,wItemInfo=info}) titles
			| itemKind /= IsRadioControl	= return (itemH,titles )
			| otherwise			= return (itemH,titles1)
			where
				fst3 (x,y,z)	= x
				itemId		= fromJust (wItemId itemH)
				title		= (itemId,True,Just [fst3 (radioItem item) | item<-radioItems (getWItemRadioInfo info)])
				(_,titles1)	= creplace (eqfst3id itemId) title titles


getradiocontrolsmarks :: [Id] -> OSWindowMetrics -> WIDS -> WindowHandle ls ps -> [(Id,Bool,Maybe Index)] -> IO (WindowHandle ls ps, [(Id,Bool,Maybe Index)])
getradiocontrolsmarks ids wMetrics wids wH@(WindowHandle {whItems=itemHs}) marks = do
	...
	where
		...
		getmarks itemH@(WItemHandle {wItemKind=itemKind,wItemInfo=info}) marks
			| itemKind /= IsRadioControl	= return (itemH,marks )
			| otherwise			= return (itemH,marks1)
			where
				itemId		= fromJust (wItemId itemH)
				mark		= (itemId,True,Just (radioIndex (getWItemRadioInfo info)))
				(_,marks1)	= creplace (eqfst3id itemId) mark marks


getcheckitems :: [Id] -> OSWindowMetrics -> WIDS -> WindowHandle ls ps -> [(Id,Bool,Maybe [String])] -> IO (WindowHandle ls ps, [(Id,Bool,Maybe [String])])
getcheckitems ids wMetrics wids wH@(WindowHandle {whItems=itemHs}) titles = do
	(ids, itemHs, titles) <- statemapWElementHandles getchecktitle ids itemHs titles
	return (wH{whItems=itemHs}, titles)
	where
		getchecktitle :: WElementHandle ls ps -> [(Id,Bool,Maybe [String])] -> IO (WElementHandle ls ps, [(Id,Bool,Maybe [String])])
		getchecktitle itemH@(WItemHandle {wItemKind=itemKind,wItemInfo=info}) titles
			| itemKind /= IsCheckControl	= return (itemH,titles )
			| otherwise			= return (itemH,titles1)
			where
				fst4 (a,b,c,d)	= a
				itemId		= fromJust (wItemId itemH)
				title		= (itemId,True,Just [fst4 (checkItem item) | item <- checkItems (getWItemCheckInfo info)])
				(_,titles1)	= creplace (eqfst3id itemId) title titles


getcheckcontrolsmarks :: [Id] -> OSWindowMetrics -> WIDS -> WindowHandle ls ps -> [(Id,Bool,Maybe [Index])] -> IO (WindowHandle ls ps, [(Id,Bool,Maybe [Index])])
getcheckcontrolsmarks ids wMetrics wids wH@(WindowHandle {whItems=itemHs}) marks = do
	(ids, itemHs, titles) <- statemapWElementHandles getmarks ids itemHs marks
	return (wH{whItems=itemHs}, marks)
	where
		getmarks :: WElementHandle ls ps -> [(Id,Bool,Maybe [Index])] -> IO (WElementHandle ls ps, [(Id,Bool,Maybe [Index])])
		getmarks itemH@(WItemHandle {wItemKind=itemKind,wItemInfo=info}) marks
			| itemKind /= IsCheckControl	= return (itemH,marks )
			| otherwise			= return (itemH,marks1)
			where
				itemId		= fromJust (wItemId itemH)
