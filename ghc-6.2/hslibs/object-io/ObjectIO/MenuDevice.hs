module MenuDevice(menuFunctions) where
...
import	MenuAccess(menuStateHandleGetMenuId)
import	StdMenuAttribute(isMenuFunction, getMenuFun, isMenuModsFunction, getMenuModsFun)
...
menuClose ps = do
	...
	(case getOSDInfoOSMenuBar osdinfo of
		...
		Just osMenuBar -> do
			...
			when (not found) (menuDeviceFatalError "MenuFunctions.dClose" "could not retrieve MenuSystemState from IOSt")				
			...
			it <- ioStGetIdTable
			let (rt1,it1) = foldr (disposeIds ioid) (rt,it) (mMenus mHs)
			ioStSetIdTable it1
	...
	where
		...
		disposeIds ioid (MenuStateHandle (MenuLSHandle {mlsHandle=mH})) ts =
			foldr (disposeMenuIds ioid) ts (mItems mH)
...
menuIO deviceEvent ps = do
	...
	when (not ok) (menuDeviceFatalError "menuFunctions.dDoIO" "could not retrieve MenuSystemState from IOSt")
	toGUI (menuIO deviceEvent ps)
	where
		menuIO :: DeviceEvent -> ps -> IOSt ps -> IO (ps,IOSt ps)
		menuIO (ReceiverEvent recLoc) ps ioState =
			let
				(_,mDevice) = ioStGetDevice MenuDevice ioState
				menus   = menuSystemStateGetMenuHandles mDevice
			in
				menuMsgIO recLoc menus ps ioState
			where		
				menuMsgIO :: RecLoc -> MenuHandles ps -> ps -> IOSt ps -> IO (ps, IOSt ps)
				menuMsgIO msgEvent menus@(MenuHandles {mMenus=mHs}) ps ioState = do
					menusMsgIO (\mHs ioState -> (ioStSetDevice (MenuSystemState menus{mMenus=mHs})) ioState) (rlParentId recLoc) recLoc mHs ps ioState
					where
						menusMsgIO :: ([MenuStateHandle ps] -> IOSt ps -> IOSt ps) ->
							      Id -> RecLoc -> [MenuStateHandle ps] -> ps -> IOSt ps -> IO (ps, IOSt ps)
						menusMsgIO build menuId recLoc [] ps ioState =
							menuDeviceFatalError "menuIO (ReceiverEvent _) _" "menu could not be found"
						menusMsgIO build menuId recLoc (msH:msHs) ps ioState =
							if menuStateHandleGetMenuId msH == menuId
							then menuStateMsgIO (\msH ioState -> build (msH:msHs) ioState) recLoc msH ps ioState
							else menusMsgIO (\msHs ioState -> build (msH:msHs) ioState) menuId recLoc msHs ps ioState
...
menuStateMsgIO :: (MenuStateHandle ps -> IOSt ps -> IOSt ps) ->
		  RecLoc -> MenuStateHandle ps -> ps -> IOSt ps -> IO (ps,IOSt ps)
menuStateMsgIO build recLoc msH@(MenuStateHandle mlsH@(MenuLSHandle {mlsState=ls,mlsHandle=mH})) ps ioState = do
    r <- menuMsgIO (\mH st ioState -> build (MenuStateHandle mlsH{mlsState=fst st,mlsHandle=mH}) ioState) rId recLoc mH (ls,ps) ioState
    let ((_,ps1),ioState) = r
    return (ps1,ioState)
    where
	...
	menuMsgIO :: (MenuHandle ls ps -> (ls,ps) -> IOSt ps -> IOSt ps) ->
		       Id -> RecLoc -> MenuHandle ls ps -> (ls,ps) -> IOSt ps -> IO ((ls,ps), IOSt ps)
	menuMsgIO build rId recLoc mH@(MenuHandle {mItems=itemHs}) ls_ps ioState = do
