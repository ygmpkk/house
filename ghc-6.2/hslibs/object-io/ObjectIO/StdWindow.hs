module StdWindow ( Windows(..), Dialogs(..), module StdWindowDef
                 , closeWindow, closeActiveWindow, setActiveWindow, getActiveWindow
                 , setActiveControl, getActiveControl, getWindowLook
                 ...
                 , setWindowTitle, setWindowCursor
                 , getWindowTitle, getWindowOk
                 , getWindowCancel, getWindowCursor
...
import ControlInternal(enablecontrols,disablecontrols)
import qualified ControlPos as CP
import Id
import IOState
import StdControlClass
import StdWindowDef
import StdWindowAttribute
import StdSystem(maxScrollWindowSize)
import StdId(getParentId)
import WindowAccess
import WindowCreate
import WindowDevice
import WindowDispose
import WindowValidate
import WindowClipState(validateWindowClipState, forceValidWindowClipState)
import qualified WindowDraw as WD
import qualified WindowUpdate as WU
import WindowDraw(drawWindowLook)
import OSPicture(Draw(..))
import WindowUpdate(updateWindowBackgrounds)
import OSWindow
import OSEvent
import OSSystem(osStripOuterSize)
import OSToolBar(OSToolbar(..))
import Scheduler(handleContextOSEvent)
import KeyFocus(getCurrentFocusItem)
			...
			if isZero then throwGUI ErrorViolateDI
			   ...
			   case maybe_okId of
			   	...
			   	Just okId -> do
				  	...
				  	(case controlIdsAreConsistent ioId okId itemHs rt it of
				  		Nothing -> throwGUI ErrorIdsInUse
				  		Just (it,rt1) -> 
				  			let it1 = snd (addIdToIdTable okId (IdParent {idpIOId=ioId,idpDevice=WindowDevice,idpId=okId}) it)
				  			in do
				  				ioStSetIdTable it1
...
--	getActiveControl returns the Id of the currently active control.
	...
	(if not found1 || not found2 || getWindowStateHandleWindowMode wsH == Modal then return ()
	      ...
	      in if not okId then return ()
		  	...
		  	(framePtr,clientPtr) = case getOSDInfoOSInfo osdInfo of
		  			...
		  			_         -> (osNoWindowPtr,osNoWindowPtr)
		      in do
			   (pos,windows) <- liftIO (exactWindowPos wMetrics wSize (Just pos) wKind Modeless windows)
...
setWindowViewSize wid reqSize = do
	(found,wDevice)	<- accIOEnv (ioStGetDevice WindowDevice)
	(if not found then return ()
	 else let
		windows	= windowSystemStateGetWindowHandles wDevice
	      ...
	      in
		if not found || getWindowStateHandleWindowKind wsH /= IsWindow then return ()
		else do
			wMetrics <- accIOEnv ioStGetOSWindowMetrics
	  		let (diffSize,viewSize,frameSize) = validateSize wMetrics reqSize wsH
			(if not diffSize then return ()
			 else do
				osdInfo <- accIOEnv ioStGetOSDInfo
				...
				wsH <- liftIO (windowStateSizeAction wMetrics (isJust activeWIDS && wId (fromJust activeWIDS)==wid) (WindowSizeActionInfo {wsWIDS=wids,wsSize=frameSize,wsUpdateAll=False}) wsH)
				appIOEnv (ioStSetDevice (WindowSystemState (setWindowHandlesWindow wsH windows1)))))
	where
