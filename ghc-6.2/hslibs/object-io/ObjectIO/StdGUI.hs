...
import {-# SOURCE #-} IOState(GUI, noLS, noLS1)
import StdIOCommon(Modifiers(..),KeyboardState(..),SliderState(..),SliderMove(..),MouseState(..),ViewFrame, ItemPos, Size(..))
import StdBitmap(Bitmap)
