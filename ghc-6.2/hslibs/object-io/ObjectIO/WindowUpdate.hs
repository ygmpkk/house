module WindowUpdate( updateWindow, updateWindowBackgrounds
...
--  
--  WindowUpdate contains the window update functions.
--  ********************************************************************************
...
import	DeviceEvents
import	WindowHandle
import	WindowAccess(getWItemCompoundInfo, getWItemCustomButtonInfo, getWItemCustomInfo,
			getCompoundContentRect, getWindowContentRect, getCompoundHScrollRect, getCompoundVScrollRect)
import	WindowClipState(validateWindowClipState)
	...
	updateControlBackgrounds wMetrics backgrRgn wH@(WindowHandle {whItems=whItems,whSelect=whSelect,whSize=whSize}) osPict = do
		...
		updateBackgrounds wMetrics wFrame ableContext backgrRgn itemHs osPict = do
			...
			updateBackground wMetrics wFrame ableContext backgrRgn itemH@(WItemHandle {wItemShow=wItemShow,wItemVirtual=wItemVirtual,wItemKind=wItemKind,wItemPos=wItemPos,wItemSize=wItemSize}) osPict
				...
				updateCompoundBackground wMetrics wFrame compoundRect ableContext backgrRgn itemH@(WItemHandle {wItemKind=IsCompoundControl,wItems=wItems,wItemPos=itemPos,wItemSize=itemSize}) osPict = do
					...
					(x,_,_,_) <- doDraw (origin-itemPos) lookPen True osPict (clipOSPicture updRgn contentRect (lookFun (if cAble then Able else Unable) updState))
					backgrRgn <- updateBackgrounds wMetrics cFrame cAble backgrRgn wItems osPict
					rectRgn <- osNewRectRgn cFrame
					diffRgn <- osDiffRgn backgrRgn rectRgn
					mapM_ osDisposeRgn [rectRgn,backgrRgn,updRgn]
...
updateRectControls wMetrics area wPtr wH@(WindowHandle {whItems=whItems,whSelect=whSelect})
	...
	updateControlsInRect wMetrics parentPtr ableContext area itemHs
		...
		updateControlInRect wMetrics parentPtr ableContext area itemH@(WItemHandle {wItemKind=wItemKind,wItemPtr=wItemPtr,wItemPos=wItemPos,wItemSize=wItemSize,wItemShow=wItemShow,wItemVirtual=wItemVirtual})
			...
			isCustomControl		= case wItemKind of
				...
				_			-> False			
			(isOSControl,updateOSControl) = case wItemKind of
	...
	--		updatecustomcontrol updates a ((Custom)Button/Compound)Control.
			...
			updateCustomControl _ parentPtr contextAble area itemH@(WItemHandle {wItemKind=IsCustomButtonControl, wItemSize=wItemSize, wItemPos=wItemPos, wItemPtr=itemPtr}) = do
			    osPict <- osGrabControlPictContext parentPtr itemPtr
			    (_,_,pen,_) <- doDraw zero (lookPen lookInfo) True osPict (accClipPicture (toRegion updArea) (lookFun lookInfo selectState updState))
			...
			updateCustomControl _ parentPtr contextAble area itemH@(WItemHandle {wItemKind=IsCustomControl,wItemSize=wItemSize,wItemPtr=itemPtr,wItemPos=itemPos}) = do
			    osPict <- osGrabControlPictContext parentPtr itemPtr			    
			    (_,_,pen,_) <- doDraw zero (lookPen lookInfo) True osPict (accClipPicture (toRegion updArea) (lookFun lookInfo selectState updState))			    
			...
			updateCustomControl wMetrics parentPtr contextAble area itemH@(WItemHandle {wItemKind=IsCompoundControl,wItems=wItems,wItemSize=itemSize,wItemPtr=itemPtr,wItemPos=itemPos}) = do
			    osPict <- osGrabControlPictContext parentPtr itemPtr			    
			    (_,_,pen,_) <- doDraw origin (lookPen lookInfo) True osPict (accClipPicture (toRegion updArea) (lookFun lookInfo selectState updState))			    
...
updateWindowBackground wMetrics pictureInitialised alsoClipRgn info@(UpdateInfo {updWIDS=WIDS{wPtr=wPtr}}) wH@(WindowHandle {whKind=whKind,whWindowInfo=windowInfo,whSize=whSize}) osPict
    ...
    | otherwise = do	    
	    let drawf = do
		curClipRgn <- pictGetClipRgn		
		(if curClipRgn==nullPtr then pictSetClipRgn else pictAndClipRgn) (clipRgn clipState)		   
		(case alsoClipRgn of
		...
		pictSetClipRgn curClipRgn
		liftIO (when (curClipRgn/=nullPtr) (osDisposeRgn curClipRgn))	    
	    (_,_,pen,_) <- doDraw zero initPen True osPict drawf
	...
	updateControls wMetrics wids contextAble updControls itemHs osPict
		...
		updateControl wMetrics wids contextAble updControls itemH@(WItemHandle {wItemShow=wItemShow}) osPict = do
			...
			updateControl' wMetrics wids contextAble updControls itemH@(WItemHandle {wItemNr=wItemNr}) osPict = do
				...
				updateControl'' wMetrics wids contextAble area itemH@(WItemHandle {wItemKind=IsCustomButtonControl}) osPict = do				    
				    (_,_,pen,_) <- doDraw zero (lookPen lookInfo) True osPict (accClipPicture (toRegion cFrame) (lookFun lookInfo selectState updState))				    
				    let info1 = info{cButtonInfoLook=lookInfo{lookPen=pen}}
				    return (itemH{wItemInfo=WCustomButtonInfo info1})
				    where
					selectState	= if (contextAble && wItemSelect itemH) then Able else Unable
				...
				updateControl'' wMetrics wids contextAble area itemH@(WItemHandle {wItemKind=IsCustomControl}) osPict = do				    
				    (_,_,pen,_) <- doDraw zero (lookPen lookInfo) True osPict (accClipPicture (toRegion cFrame) (lookFun lookInfo selectState updState))
				    let info1 = info{customInfoLook=lookInfo{lookPen=pen}}
				    return (itemH{wItemInfo=WCustomInfo info1})
				    where
				...
				updateControl'' wMetrics wids contextAble area itemH@(WItemHandle {wItemKind=IsCompoundControl}) osPict = do				    
				    (_,_,pen,_) <- doDraw origin (lookPen lookInfo) True osPict (accClipPicture (toRegion cFrame) (lookFun lookInfo selectState updState))
				    let info1 = info{compoundLookInfo=compLookInfo{compoundLook=lookInfo{lookPen=pen}}}
