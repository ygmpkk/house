...
import ControlPos(moveWindowViewFrame)
import IOState
import OSEvent
import OSWindow
...
openModalWindow wId (WindowLSHandle {wlsState=wlsState,wlsHandle=wH}) ps = do
    ...
    when (not found) (windowcreateFatalError "openModalWindow" "could not retrieve WindowSystemState from IOSt")
    ...
    (_,_,_,_,wH,windows1) <- liftIO (validateWindow wMetrics osdinfo wH windows)
    ...
    let modalWIDS       = getWindowHandlesActiveModalDialog windows1
    let windows2       	= addWindowHandlesActiveWindow wsH windows1  -- frontmost position is assumed by system
    ...
    when (not noError) (throwGUI (OtherError "could not create modal dialog"))
    return (ps, finalLS)
    where
...
-}
		...
		(found,wDevice) <- accIOEnv (ioStGetDevice WindowDevice)
		when (not found) (windowcreateFatalError "getFinalModalDialogLS" "could not retrieve WindowSystemState from IOSt")
		let windows = windowSystemStateGetWindowHandles wDevice
...
getStackBehindWindow index wsHs@(WindowHandles {whsWindows=whsWindows})
    = (wPtr wids,wsHs {whsWindows=before++(wsH:after)})
    where
