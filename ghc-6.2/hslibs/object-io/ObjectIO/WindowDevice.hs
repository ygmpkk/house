module WindowDevice ( windowFunctions
...
import WindowEvent
import WindowHandle
import WindowUpdate (updateWindow, updateWindowBackgrounds)
import KeyFocus (setNewFocusItem, setNoFocusItem)
import OSEvent (createOSLooseMouseEvent, createOSLooseKeyEvent, createOSDeactivateControlEvent)
import OSRgn (osGetRgnBox)
import OSPicture (Draw(..), pictScroll)
...
windowClose ps
	= do {	(found,wDevice) <- accIOEnv (ioStGetDevice WindowDevice);
		...
		do
			...
			let (freeRIdss, freeIdss, _, finalLS) = unzip4 disposeInfo
			idtable <- ioStGetIdTable
			ioStSetIdTable (snd (removeIdsFromIdTable (concat freeIdss) idtable))
	  ...
	  where
		disposeWindowStateHandle' :: OSDInfo -> WindowStateHandle ps -> (Maybe InputTrack, ps) -> GUI ps (([Id],[Id],[DelayActivationInfo],[FinalModalLS]),(Maybe InputTrack, ps))
		disposeWindowStateHandle' osdinfo wsH (inputTrack, ps) = do
			(a,b,c,d,inputTrack,ps) <- disposeWindowStateHandle osdinfo inputTrack wsH ps
			return ((a,b,c,d),(inputTrack,ps))
...
windowIO deviceEvent ps
	...
	where
		windowIO' :: DeviceEvent -> ps -> GUI ps ps
		windowIO' (ReceiverEvent recLoc) ps = do
			...
			toGUI (windowStateMsgIO recLoc wsH windows1 ps)
			where				
				wId	= rlParentId recLoc
		...
		windowIO' (ControlMouseAction info) ps = do
			(_,wDevice) <- accIOEnv (ioStGetDevice WindowDevice)
		  	...
		  	toGUI (windowStateControlMouseActionIO info wsH windows ps)		  	
		...
		windowIO' (WindowCreateControls wids) ps = do
			...
			(cPtr,wsH2)     <- liftIO (createDialogControls wMetrics wsH1)
			delayInfo       <- liftIO (osActivateControl (wPtr wids) cPtr)
			bufferDelayedEvents delayInfo
			let windows2     = addWindowHandlesActiveWindow wsH2 windows1
			appIOEnv (ioStSetDevice (WindowSystemState windows2))
			return ps
			where
				...
				createDialogControls wMetrics wsH@(WindowStateHandle wids (Just wlsH@(WindowLSHandle {wlsHandle=wH}))) = do
					...
					return (itemPtr,WindowStateHandle wids (Just (wlsH {wlsHandle=wH1})))
		...
		windowIO' (WindowSizeAction info) ps = do
			...
			wMetrics <- accIOEnv (ioStGetOSWindowMetrics)
			wsH <- liftIO (windowStateSizeAction wMetrics (isJust activeWIDS && (wId (fromJust activeWIDS)) == winId) info wsH)
			let windows2 = setWindowHandlesWindow wsH windows1
			appIOEnv (ioStSetDevice (WindowSystemState windows2))
			return ps
...
windowStateMsgIO :: RecLoc -> WindowStateHandle ps -> WindowHandles ps -> ps -> IOSt ps -> IO (ps, IOSt ps)
windowStateMsgIO recLoc wsH@(WindowStateHandle wids (Just wlsH@(WindowLSHandle {wlsState=ls,wlsHandle=wH}))) windows ps ioState = do
...
windowStateSizeAction :: OSWindowMetrics -> Bool -> WindowSizeActionInfo -> WindowStateHandle ps -> IO (WindowStateHandle ps)
windowStateSizeAction wMetrics isActive info@(WindowSizeActionInfo {wsWIDS=WIDS{wPtr=wPtr},wsSize=wsSize,wsUpdateAll=wsUpdateAll}) (WindowStateHandle wids (Just wlsH@(WindowLSHandle {wlsHandle=wH}))) =
    let 
    ...
    in do
	setWindowScrollThumbValues hasHScroll wMetrics wPtr True  ((w newContentSize)+1) (x oldOrig) (x newOrig) newSize'
	setWindowScrollThumbValues hasVScroll wMetrics wPtr False ((h newContentSize)+1) (y oldOrig) (y newOrig) newSize'
	wH <- resizeControls wMetrics isActive wsUpdateAll (wsWIDS info) oldOrig oldContentSize newContentSize wH1
	return (WindowStateHandle wids (Just wlsH{wlsHandle=wH}))
    where
	oldSize				= whSize wH
