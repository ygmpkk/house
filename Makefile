...
patch-stamp: standalone.diff
	@if test ! -d $(GHCTOP); then \
		echo "No 'ghc-6.2' directory found." ; \
		echo "Please use 'make boot' to download and unpack GHC 6.2" ; \
		echo "(or do it yourself if you have ghc-6.2-src.tar.bz2 handy)."; \
		exit 1; \
	fi
...
glafp-utils-stamp: configure-stamp
	(cd $(GHCTOP)/glafp-utils && $(MAKE) boot && $(MAKE) all)
	touch $@

ghc-stamp: glafp-utils-stamp
	(cd $(GHCTOP)/ghc && $(MAKE) boot && $(MAKE) all)
	touch $@

# Quick fix:
ghc-inplace:
	echo "#!/bin/sh" >ghc-6.2/ghc/compiler/stage1/ghc-inplace
	echo "exec $(GHCTOP)/ghc/compiler/stage1/ghc-6.2 -B$(GHCTOP) \"\$$@\"" >>ghc-6.2/ghc/compiler/stage1/ghc-inplace
...
distclean:
	...
	rm -rf ghc-6.2 ghc-6.2-src.tar.bz2 ghc-stamp glafp-utils-stamp \
