...
type Connections m = FiniteMap (Port,Peer) (TCPPacketIn->m ())
type Listeners m = FiniteMap Port (Listening m)
...
server debugIO myIP iface reqChan =
  ...
  where
    ...
    init = T {listeners=emptyFM, connections=emptyFM}
    ...
    -- State updates:
    listen   port accept s@T{listeners=l} = s{listeners=addToFM l port accept}
    unlisten port        s@T{listeners=l} = s{listeners=delFromFM l port}
    connect    c fwd  s@T{connections=cs} = s{connections=addToFM cs c fwd}
    disconnect c      s@T{connections=cs} = s{connections=delFromFM cs c}
      ...
      do let packet = IPv4.content ipPacket
	     ...
	     dropit =
	       debug $ "Dropped packet from "++show peer ++" to "++show me
			++ "\n"++show ipPacket
	 optcon <- gets (flip lookupFM (port,peer).connections)
	 case optcon of
	   Just toConnection -> do --debug $ "Forwarding "++show c
				   lift $ toConnection ipPacket
	   _ -> do optlistener <- gets (flip lookupFM port . listeners)
		   case optlistener of
    ...
    pickPort = do T{listeners=l,connections=c} <- get
		  let inuse = keysFM l++map fst (keysFM c) -- duplicates, slow?
